...
class StageTwoHttpTests(unittest.TestCase):
    ...
    def test_status_and_lists_are_readonly_and_static_ui_is_package_local(self) -> None:
        ...
        self.assertIn('<img src="/static/icon.png" alt="">', page.text)
        self.assertIn("<title>FolioHook · 寻章 — 本地知识库</title>", page.text)
        self.assertIn("FolioHook <span>· 寻章</span>", page.text)
        self.assertIn("切换文献库", page.text)
        self.assertIn("拖入或选择", script.text)
        ...
        self.assertIn("未人工核验", script.text)
