...
开发与验收协作从 [项目任务书](./docs/项目任务书.md)、[执行看板入口](./docs/执行看板.md) 和 [仓库协作约定](./AGENTS.md) 开始；当前任务进度以本机唯一看板为准。

> **普通用户入口：** 从 [亲手操作教程](./docs/亲手操作教程.md) 开始，用现成练习文件一步一步完成；安装、参数与报错需要时再查 [设置与使用参考](./docs/使用指南.md)。教程对应当前主工作区已纳入的正式 UI 与近期修复；它仍是源码开发版，不是已打包的安装包。需要安装协助时，把 [本地 Agent 完整安装与连接提示词](./docs/local-agent-setup.md) 整段复制给能操作这台 Mac 文件和终端的本地 Agent；它会分阶段检查、安装、启动合成样例和自检，并只在你确认后配置选定连接。

项目目标是“可审计的本地文献证据工具 + 八个只读 MCP 工具”，不是通用聊天软件。当前已完成历史阶段一至阶段四，以及下一轮产品化阶段 0–9 的本地实现与离线验收合同；真实模型、客户端与 Tunnel 已完成一次合成样例真实链路验收，但仍需在每位使用者本机分别验收，不代表一般检索质量或全新 Mac 安装已经通过。历史阶段四提供的是源码项目里的 Apple Silicon macOS 开发版 `.command` 入口，不是自包含、已签名或已公证的 App/DMG/pkg。
...
- 生产 Web/stdio 入口已接入阿里云北京地域接口：`qwen3.8-flash` 改写、`qwen3.7-text-embedding` 向量、`qwen3-rerank` 重排。Key 由管理页保存到本应用独立的 macOS 钥匙串项目；保存、状态查看、向量发送量预览及默认 BM25 不读取 Key 或调用模型。所选快照须先明确构建真实向量，再显式使用 enhanced；模型请求零自动重试，失败返回实际已发送次数，成功/失败响应中的服务商用量不等于最终账单。服务端模型 revision 由供应商管理，不宣称锁定模型权重。
...
- Stage 9 的离线模拟仍单独保留。生产管理页另有真实 Tunnel 入口，使用应用根内的官方 `bin/tunnel-client` 和同一个 `mcp-server` shim；运行 Key 从钥匙串经匿名管道传入，不进入参数、环境变量或文件。需填写本项目 Tunnel ID 并接受官方客户端断线退避重连；应用不自动重启，只停止自身进程组。关闭本机服务会停止其 Tunnel；本地连接健康不代表 ChatGPT 已发现工具。
...
这些文本只解析固定的 `$HOME` 应用位置，不包含 key、用户绝对路径、虚拟环境路径或可填写的 env。Finder 入口会原子更新权限为 0700 的 shim；管理页只检查模板及解释器是否就绪，不执行真实入口。点击复制后仍是“未配置”，需要在客户端完成配置。本地 BM25 与八工具不需要 OpenAI API key。ChatGPT web 使用下方真实 Tunnel 接入区域，独立于 Stage 9 的离线模拟。
...
下一轮阶段 0–9 与上面的历史编号分开：阶段 0 冻结合同；阶段 1–9 已依次完成多资料库、双级快照生命周期、库内增量对象复用、离线同配置向量复用、小白导入界面、可替换三角色增强链、合成搜索质量验收、本地 MCP 向导与真实 stdio 离线自检，以及 Tunnel 离线模拟向导。完整顺序与最小验收见 [ARCHITECTURE.md](./ARCHITECTURE.md#下一轮产品化阶段-0-合同)。后续用户已授权真实接入，当前分支新增上述生产适配与本机填写入口；离线测试通过不能代替填 Key 后的真实调用与 ChatGPT 工具发现验收。
...
真实运行所需的 Tunnel ID、运行凭据、OpenAI Platform 组织与 ChatGPT 工作区关联，以及创建用 `Read + Manage`、运行／选择用 `Read + Use` 权限，见 [OpenAI Secure MCP Tunnel 官方说明](https://developers.openai.com/api/docs/guides/secure-mcp-tunnels)。填写入口和真实适配代码已提供，实际模型质量、Tunnel 认证及 ChatGPT 工具发现仍须分别以真实结果验收；历史离线报告不变。
