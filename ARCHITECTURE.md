...
`python -m literature_evidence_mcp.quality_eval` 是独立评测入口，不进入 Web 或 MCP。每次运行只在一次性临时目录构造两座物理隔离资料库、同名来源、同库两个不同快照、合成 Markdown 与两页文本层 PDF，再为三个快照生成阶段 4 离线向量 artifact；退出时清理整个临时根，不读取默认用户应用根，也不改变仓库业务树。
