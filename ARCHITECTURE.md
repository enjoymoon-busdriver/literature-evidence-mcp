...
下一轮使用阶段 0–9 编号，与上文已经完成的历史阶段一至四不是同一组阶段。阶段 0 只冻结合同，不修改运行时代码、快照、SQLite schema 或 MCP schema；阶段 1–9 必须依次实现和验收，不能把后阶段能力提前混入前阶段。
...
| 1. 多资料库 | 用户可创建、命名、重命名和描述资料库；持久保存稳定 `library_id` | 两个资料库重启后仍可独立识别和切换；同名内容不混库；AI/MCP 没有写入口 |
| 2. 快照生命周期与 AI 选库/选快照 | 候选、当前和上次成功快照状态；`retrieval_status` 完成“列库→列快照→核验”；其余工具显式使用两级 ID | 失败构建不影响旧成功快照；两个库有相似关键词时，八工具不能串库或串版本；MCP 数量仍为八 |
| 3. 增量存储 | 每库独立共享对象区；快照保存完整成员清单并引用内容寻址的原文件和解析块 | S1=A+B、S2=A+B+C 时共享对象区只新增 C（S2 自身清单和独立 BM25 索引除外）；S1 仍完整表示 A+B；修改 B 或解析/分块配置只新增变化对象，旧对象和旧快照不变且不得误复用 |
| 4. 同配置向量复用 | 冻结语料向量身份和缓存逻辑；只用确定性的离线假模型测试 | 同配置下 S2 只为 C 生成向量；任一文本、模型、版本、维度、指令或预处理变化都不得误复用；不发真实请求 |
| 5. 小白导入界面 | 在现有本地网页中创建/切换资料库，拖放或选择文件，显示逐文件阶段、失败原因、快照差异和新增/复用统计 | 从空白状态仅用界面建立两个库和多个快照；无需终端；部分失败不会发布残缺快照或破坏旧快照 |
| 6. 三模型增强搜索 | 问题改写、向量召回、候选重排的一套可替换传输层；BM25 与增强模式分离 | 用离线假服务验证调用次数、外发边界、首错停止、无自动重试和无静默回退；真实模型、真实密钥和真实钥匙串不在本轮验收 |
